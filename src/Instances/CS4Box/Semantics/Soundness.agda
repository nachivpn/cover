...
module Instances.CS4Box.Semantics.Soundness where
...
module Proof
  (𝒜 : CS4BoxAlgebra)
  (open CS4BoxAlgebra 𝒜 using (Carrier))
  (V𝕡 : Atom → Carrier)
  where
  ...
  ⟦-⟧-sound {Δ} {Γ} {a} (hypᵍ x)
    = ≤-trans (proj₁' (◻' ⟦ Δ ⟧c) ⟦ Γ ⟧c) (≤-trans ◻'-extract (⟦-⟧-sound-hyp x))
  ⟦-⟧-sound {Δ} {Γ} {_} ⊤-I
    = unit' ⟦ Δ , Γ ⟧c₂
  ⟦-⟧-sound {_} {_} {a} (⊥-E t)
