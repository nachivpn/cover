...
-- Dual Context S4 calculus
module Instances.CS4Box.System where
...
data _⨾_⊢_ (Δ Γ : Ctx) : Form → Set where
  hypˡ   : (x : Var Γ a) → Δ ⨾ Γ ⊢ a
  hypᵍ   : (x : Var Δ a) → Δ ⨾ Γ ⊢ a
...
wkTm i1 i2 (hypˡ x)      = hypˡ (wkVar i2 x)
wkTm i1 i2 (hypᵍ x)      = hypᵍ (wkVar i1 x)
wkTm i1 i2 ⊤-I           = ⊤-I
...
wkTm i1 i2 (◻-I t)       = ◻-I (wkTm i1 base t)
